    ...
    render() {
        ...
        const renderEmptyImages = (length) => {
            for (let i = 0; i < length; i++) {
                emptyImage.push(
                    <img key={i} src={empty} className="sub-img-detail-product" alt="sub img" />
                );
            }
            return emptyImage;
        };
                    ...
                    <article className="detail-product">
                        <div className="img-detail-product">
                            <div className="sub-img-product" >
                                    ...
                                    (
                                        product.images.map((ele, index) => {
                                            return <img key={ele.id} onClick={(e) => this.handleClickSubImage(e)}
                                                src={ele.urlImage} className="sub-img-detail-product" alt="sub img" />
                                        })
                                    )
                                    : (
                                        <>
                                            <img onClick={(e) => this.handleClickSubImage(e)}
                                                src={empty} className="sub-img-detail-product" alt="sub img" />
                                ...
                                {product.images ? renderEmptyImages(5 - product.images.length) : ""}
                            </div>
                            ...
                            <div className="body-info-detail-product">
                                ...
                                <div className="title-detail-item">
                                    ...
                                    <span style={{ color: '#f5222d', fontSize: '18px' }}>
                                        <b>{parsePriceForSale.parsePriceSale(product.price, product.sale) || 0} đ</b>
                                    </span>
                                </div>
                                <div className="title-detail-item">
                                    <span className="title-detail"><b>Giá thị trường:</b></span>
                                    <span style={{ fontSize: '18px', textDecoration: 'line-through' }}>
                                        {parsePriceForSale.parsePrice(product.price) || 0}<b> đ</b>
                                    </span>
                                </div>
                                <div className="title-detail-item">
                                    <span className="title-detail"><b>Size:</b></span>
                                    <Tag color="default"><b>{sizes[product.size]}</b></Tag>
                                </div>
                                <div className="title-detail-item">
